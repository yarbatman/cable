...
export default function Kabel(){
  ...
  const[mentionPopup,setMentionPopup]=useState(null);const[manualHandle,setManualHandle]=useState("");
  ...
  useEffect(()=>{if(!mentionPopup)return;const h=e=>{if(!e.target.closest("[data-mpopup]")&&!e.target.closest("[data-mchip]")){setMentionPopup(null);setManualHandle("")}};document.addEventListener("mousedown",h);return()=>document.removeEventListener("mousedown",h)},[mentionPopup]);
  ...
  const handleSelectCandidate=(name,platformKey,candidate)=>{setMentions(prev=>{const u={...prev};if(u[name]&&u[name][platformKey]){u[name]={...u[name]};u[name][platformKey]=u[name][platformKey].map(c=>({...c,selected:c.handle===candidate.handle}))}return u});const oldSel=mentions[name]?.[platformKey]?.find(c=>c.selected);if(oldSel)setContents(prev=>({...prev,[platformKey]:prev[platformKey].replace(oldSel.handle,candidate.handle)}));setMentionPopup(null);setManualHandle("");setToast({message:name+" \u2192 "+candidate.handle+" on "+PLATFORMS[platformKey].name,type:"success"})};
  const handleManualOverride=(name,platformKey,handle)=>{
    if(!handle.trim())return;const h=handle.trim().startsWith("@")?handle.trim():"@"+handle.trim();
    const newCandidate={handle:h,name,bio:"Manually entered",selected:true,manual:true};
    const oldSel=mentions[name]?.[platformKey]?.find(c=>c.selected);
    setMentions(prev=>{const u={...prev};if(!u[name])u[name]={};u[name]={...u[name]};const existing=(u[name][platformKey]||[]).map(c=>({...c,selected:false}));u[name][platformKey]=[newCandidate,...existing.filter(c=>c.handle!==h)];return u});
    if(oldSel)setContents(prev=>({...prev,[platformKey]:prev[platformKey].replace(oldSel.handle,h)}));
    setMentionPopup(null);setManualHandle("");setToast({message:name+" \u2192 "+h+" on "+PLATFORMS[platformKey].name,type:"success"});
  ...
  return<div style={{minHeight:"100vh",background:C.bg,color:C.text}}>
    ...
    {mentionPopup?.type==="select"&&(()=>{const r=mentionPopup.chipRect;const w=340;let left=r.left+r.width/2-w/2;left=Math.max(12,Math.min(left,window.innerWidth-w-12));const top=Math.min(r.bottom+8,window.innerHeight-460);
    return<div data-mpopup style={{position:"fixed",left,top,width:w,zIndex:9999,background:"#fff",borderRadius:16,overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,0.22), 0 2px 10px rgba(0,0,0,0.06)",border:"1px solid "+C.borderLight,animation:"dropIn 0.15s ease"}}>
      <div style={{padding:"12px 16px",fontFamily:F.ui,fontSize:10,fontWeight:700,color:C.sub,textTransform:"uppercase",letterSpacing:"0.08em",background:C.muted,borderBottom:"1px solid "+C.borderLight,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <span onClick={()=>{setMentionPopup(null);setManualHandle("")}} style={{fontSize:13,color:C.sub,cursor:"pointer",textTransform:"none",letterSpacing:0,padding:"2px 6px",borderRadius:4,fontWeight:400}} onMouseEnter={e=>e.currentTarget.style.background=C.borderLight} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>&times;</span>
      </div>
        ...
        {mentionPopup.candidates?.map((c,i)=>{const initials=(c.name||mentionPopup.name).split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase();return(
          <button key={i} onClick={()=>handleSelectCandidate(mentionPopup.name,mentionPopup.platformKey,c)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"9px 10px",background:c.selected?C.mention+"08":"transparent",border:c.selected?"2px solid "+C.mention+"25":"2px solid transparent",borderRadius:10,cursor:"pointer",textAlign:"left",transition:"all 0.12s",marginBottom:2}}
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontFamily:F.ui,fontSize:12.5,fontWeight:650,color:C.text}}>{c.name}</span>{c.selected&&<span style={{fontSize:9,color:"#fff",background:C.success,padding:"1px 7px",borderRadius:5,fontFamily:F.ui,fontWeight:700}}>ACTIVE</span>}{c.manual&&<span style={{fontSize:9,color:C.accent,fontFamily:F.ui,fontWeight:600}}>Manual</span>}</div>
      ...
      <div style={{padding:"10px 14px 12px",background:C.muted,borderTop:"1px solid "+C.borderLight}}>
        <div style={{fontFamily:F.ui,fontSize:10,fontWeight:600,color:C.sub,marginBottom:6}}>Can't find them? Type the handle manually:</div>
        <div style={{display:"flex",gap:6}}>
          <input data-mpopup value={manualHandle} onChange={e=>setManualHandle(e.target.value)} placeholder={"@username"} onKeyDown={e=>e.key==="Enter"&&handleManualOverride(mentionPopup.name,mentionPopup.platformKey,manualHandle)}
            style={{flex:1,padding:"8px 10px",background:"#fff",border:"1.5px solid "+C.borderLight,borderRadius:8,fontFamily:F.mono,fontSize:12,color:C.text,outline:"none"}}
            onFocus={e=>e.target.style.borderColor=C.mention+"60"} onBlur={e=>e.target.style.borderColor=C.borderLight}/>
          <button onClick={()=>handleManualOverride(mentionPopup.name,mentionPopup.platformKey,manualHandle)} disabled={!manualHandle.trim()}
            style={{padding:"8px 16px",background:manualHandle.trim()?C.mention:C.border,color:manualHandle.trim()?"#fff":C.sub,border:"none",borderRadius:8,fontFamily:F.ui,fontSize:11,fontWeight:700,cursor:manualHandle.trim()?"pointer":"default",transition:"all 0.15s",flexShrink:0}}>Save</button>
        </div>
        {mentionPopup.platformKey==="linkedin"&&<div style={{marginTop:8,textAlign:"center"}}><span onClick={()=>{const n=mentionPopup.name;setMentionPopup(null);setManualHandle("");setTimeout(()=>setLinkedInUrlModal(n),50)}} style={{fontFamily:F.ui,fontSize:10.5,color:C.accent,fontWeight:600,cursor:"pointer"}}>Or paste a LinkedIn profile URL &rarr;</span></div>}
      </div>
    </div>})()}

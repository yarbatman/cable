...
For each platform, replace the **Name** with the correct @handle for that platform. Here are the resolved handles:
${Object.entries(mentions).map(([name, handles]) =>
  `- "${name}": ${Object.entries(handles).map(([p, h]) => `${p}=${h}`).join(", ")}`
).join("\n")}
If a handle is not available for a platform, use the person's full name without asterisks.
The @handle should appear naturally in the text — don't force it into an awkward position.`;
  }
